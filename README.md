...
- 知识库管理：JWT 鉴权代理 Dify datasets 生命周期（创建/列表/删除/按文本建文档/索引状态），economy 关键词索引保证默认部署无需 embedding 供应商
...
- 运行边界：知识检索、子工作流、MCP 工具节点仅支持发布后由 Dify 云端执行；画布本地试运行（浏览器 runtime-js）不实现这三类节点，包含它们的工作流点击试运行会得到明确报错而不是错误结果
...
# 管理后台各 tab → 个人中心文件管理 → 深色模式切换 → 退出，全程截图存证
...
按影响面看，剩余差距的优先补齐顺序应是：高质量向量索引（embedding 供应商接入与召回调优）、数据库节点、多轮会话、审批等待、OAuth 与团队凭据共享；Python 应在服务端沙箱、资源配额和安全回归完善后开放。以上均为差距与建议，不代表当前已实现。
