...
`test:platform` 同时运行 `test:fuzz`：这是固定种子 `0x5eedc0de` 的 20,000 组 FlowGram-to-Dify 确定性回归（10,000 组合法图、10,000 组应拒绝的非法图）。它验证 DSL 序列化、节点与连线引用完整性、Start 输入默认值、有限费用估算，以及环、孤儿节点、重复边、缺少或重复 Start、无效条件端口和异常载荷的 400 级准入拒绝。
