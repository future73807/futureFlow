...
按影响面看，剩余差距的优先补齐顺序应是：高质量向量索引（embedding 供应商接入与召回调优）、Python 节点发布后执行（见下）、OAuth 与团队凭据共享、触发器与任务的失败重试策略；多轮会话属于聊天机器人能力而非工作流引擎能力，审批与等待只在需要人工干预的场景下才值得做。以上均为差距与建议，不代表当前已实现。
