...
export type JavaScriptContractAnalysis = {
  identifiers: Set<string>;
  mainDeclarationCount: number;
...
const skipTemplateExpression = (source: string, start: number): number => {
  ...
  while (cursor < source.length && depth > 0) {
    ...
    if (char === '\'' || char === '"') {
      ...
      previous = { kind: 'literal', value: source.slice(cursor, end), start: cursor, end, braceDepth: 0 };
      cursor = end;
      continue;
    }
    if (char === '`') {
    ...
    previous = { kind, value: source.slice(cursor, end), start: cursor, end, braceDepth: 0 };
    cursor = end;
  }
  return cursor;
};
...
const tokenizeExecutableJavaScript = (source: string): JavaScriptToken[] => {
  ...
  while (cursor < source.length) {
    ...
    if (char === '\'' || char === '"') {
      const end = skipQuotedString(source, cursor, char);
      tokens.push({ kind: 'literal', value: source.slice(cursor, end), start: cursor, end, braceDepth });
      cursor = end;
      continue;
    }
    if (char === '`') {
    ...
    tokens.push({ kind: 'punctuator', value: char, start: cursor, end: cursor + 1, braceDepth });
...
const isDeclarationBoundary = (token: JavaScriptToken | undefined): boolean =>
  !token || token.value === ';' || token.value === '}';

export const analyzeSynchronousJavaScript = (source: string): JavaScriptContractAnalysis => {
  ...
  tokens.forEach((token, index) => {
    ...
    if (
      ...
      && token.braceDepth === 0
      && isDeclarationBoundary(tokens[index - 1])
...
export const assertSynchronousJavaScript = (
  source: string,
  label: string,
): JavaScriptContractAnalysis => {
...
export const uniqueJavaScriptIdentifier = (
  identifiers: Set<string>,
  base: string,
): string => {
  let candidate = base;
...
export const normalizeCodeOutputSchema = (outputs: any, label: string): CodeOutputSchema => {
  ...
  const normalizedProperties = Object.keys(properties || {}).length > 0
    ? Object.fromEntries(
        Object.entries(properties).map(([key, schema]) => [
          key,
          normalizeSchema(schema, `${label}输出“${key}”`),
        ]),
      )
...
export const buildOutputContractRuntime = (
  identifiers: Set<string>,
  schema: CodeOutputSchema,
  label: string,
): {
  declarations: string;
  validateExpression: (valueExpression: string) => string;
} => {
  const contractName = uniqueJavaScriptIdentifier(identifiers, '__futureFlowOutputContract');
  const typeName = uniqueJavaScriptIdentifier(identifiers, '__futureFlowValueType');
  const plainObjectName = uniqueJavaScriptIdentifier(identifiers, '__futureFlowIsPlainObject');
  const freeName = uniqueJavaScriptIdentifier(identifiers, '__futureFlowValidateFreeValue');
  const validateName = uniqueJavaScriptIdentifier(identifiers, '__futureFlowValidateOutput');
  const escapedLabel = JSON.stringify(label);
  const declarations = `
const ${contractName} = ${JSON.stringify(schema)};
...
function ${validateName}(schema, value, path) {
  ...
  return {
    declarations,
    validateExpression: (valueExpression: string) =>
      `if (${valueExpression} === null) throw new Error(${escapedLabel} + '输出必须是对象');
  ${validateName}(${contractName}, ${valueExpression}, ${escapedLabel} + '输出')`,
  };
};
