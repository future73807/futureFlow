...
export const NodeDescriptions: Record<string, string> = {
  ...
  knowledge: '在已创建的知识库中检索与查询语句最相关的内容片段。',
  subworkflow: '把另一个已发布工作流作为节点复用，发布时内联展开。',
  mcp: '调用已注册 MCP 服务器上的工具；凭据保存在网关，不出画布。',
  database: '对 PostgreSQL 执行只读 SELECT 查询，本地试运行时经网关代理真实执行，最多返回 200 行。',
  python: '在本机 Python 3 中执行 main({params}) 并返回结果；本地试运行时经网关代理真实执行，15 秒超时。',
  variable: '新建变量，或修改流程中已有变量的值。',
  'variable-aggregator': '把多个分支的输出聚合起来：每个分组返回第一个非空的值。',
  condition: '连接多个下游分支，满足条件时执行对应分支。',
  'multi-condition': '多条件分支，支持更复杂的条件逻辑。',
  loop: '循环处理字符串或数字数组；最多 20 项，每项执行一次同步 JavaScript。',
