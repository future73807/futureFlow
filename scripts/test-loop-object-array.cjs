    ...
    nodes: [
      ...
      },
      {
        ...
        data: {
          ...
          script: {
            language: 'javascript',
            content: "function main() { return { list: [{ name: \"a\", score: 10 }, { name: \"b\", score: 20 }, { name: \"c\", score: 30 }, { name: \"d\", score: 50 }] }; }",
          },
          outputs: {
            type: 'object',
            properties: { list: { type: 'array', items: { type: 'object', properties: { name: { type: 'string' }, score: { type: 'number' } } }, title: '对象数组' } },
          },
        },
      },
      {
        ...
        blocks: [
          { id: 'loop_start', type: 'block-start', meta: { position: { x: 0, y: 300 } }, data: {} },
          {
            ...
            data: {
              ...
              script: {
                language: 'javascript',
                content: 'function main({ params }) {\n  return { result: params.item.score * 2 };\n}',
              },
...
async function main() {
  ...
  if (output) {
    ...
    const ok = JSON.stringify(output).includes('[20,40,60,100]');
    console.log(ok ? '[PASS] 循环输出 [20,40,60,100] —— 跑通' : '[CHECK] 期望输出 [20,40,60,100]');
  } else {
    console.log('WARN: 未取到运行结果，请手动核对');
  }
  await browser.close();
  // 清理本套件创建的固定名工作流（此前不清理，库里堆了 6 个）

  reportCleanup(
    await cleanupTestWorkflows({ gateway: GATEWAY, token: await apiLogin(), names: ['循环对象数组试运行'] }),
    '本套件创建的工作流',
  );
}
